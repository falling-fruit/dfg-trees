...
class Source {
  ...
  async get(overwrite = false) {
    ...
    try {
      ...
      if (this.props.download) {
        paths.push(...(await this.getFiles()))
      }
      await this.execute()
  ...
  */
  ...
  /**
   * Download data from a valid WFS url.
   * valid = has service=WFS, and version is 1.0, 1.1, or 2.0
   * https://data.gov.au/geoserver/tree-register/wfs?service=WFS&version=2.0.0&request=GetFeature&typeNames=ckan_13079796_1b3a_448d_8a09_d7fda5611c5f&srsName=EPSG:4326
   * @param {string} wfs_url - wfs url to download data from.
   */
  downloadFromWFS(wfs_url) {
    let isServiceNameWFS = wfs_url.includes("service=WFS")
    let isVersion1_0_0 = wfs_url.includes("version=1.0.0")
    let isVersion2_0_0 = wfs_url.includes("version=2.0.0")
    let isVersion1_1_0 = wfs_url.includes("version=1.1.0")
    let isUrlValid = isServiceNameWFS & (isVersion1_0_0 || isVersion1_1_0 || isVersion2_0_0)
    // returns 1 or 0. 1= true, 0 = false
